  ...
  if (!req.body.organisations && model.schema.path('organisations')) {
    switch (tokenType) {
      ...
      case 'client': {
        ...
        if (!req.params.id) {
          if (!req.body.email) {
            return res.status(400).send('Must have an email to create or update user');
          }
        ...
        if (existingUser && existingUser.organisations) {
          ...
          const newOrgs = _.map(_.unionBy([tokenOrg], existingOrgs, _.toString), id => id.toString());
          req.body.organisations = newOrgs;
        }
      } break;
      default:
        break;
...
export const contextFilter = async function contextFilter(
  ...
  console.log('filter to this', filter);
