...
describe('organisation model', () => {
  ...
  it('should properly create organisation', async () => {
    ...
    const userAfterCreateOrganisation = await User.findOne({
      email: user.email
    });

    const createdUsersOrganisation = userAfterCreateOrganisation
      .organisations
      .find(id => id.toString() === organisation._id.toString());
    const createdUsersOrganisationSetting = userAfterCreateOrganisation
  ...
  it('should properly delete organisation', async () => {
    ...
    const userAfterDeleteOrganisation = await User.findOne({
      email: user.email
    });

    const deletedUsersOrganisation = userAfterDeleteOrganisation
      .organisations
      .find(id => id.toString() === organisation._id.toString());
    const deletedUsersOrganisationSetting = userAfterDeleteOrganisation
      .organisationSettings
      .find(orgSetting =>
        orgSetting.organisation.toString() === organisation._id.toString()
      );
