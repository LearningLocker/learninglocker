...
/**
 * Validator callback definition for `validateStructureWrapper`
 * @see validateStructureWrapper
 * @callback validateStructureWrapperCallback
 *  @param {PersonasImportStructure} structure
 */

/**
 * Helper function to avoid validation double run. If we have a valid data received,
 * just skip validation by returning `true`
 * @see https://github.com/Automattic/mongoose/issues/8495
 * @param {PersonasImportStructure} structure
 * @param {validateStructureWrapperCallback} [validator]
 * @returns {boolean} - `true` in case of valid structure
 */
const validateStructureWrapper = (structure, validator) => {
...
const schema = new mongoose.Schema({
  ...
  structure: {
    ...
    validate: [
      {
        validator: (value) => {
          if (!validateStructureWrapper(value)) {
            return true;
          }
      ...
      {
        validator: value => validateStructureWrapper(value, validatePrimaryUnique),
        message: 'Primary must be unique'
      },
      {
        validator: value => validateStructureWrapper(value, validatePrimaryNumber),
        message: 'Primary order must be an Integer'
      },
      {
        validator: value => validateStructureWrapper(value, validateOneFullName),
        message: 'You can not select multiple "Full name"',
      },
      {
        validator: value => validateStructureWrapper(value, validateOneFirstName),
        message: 'You can not select multiple "First name"',
      },
      {
        validator: value => validateStructureWrapper(value, validateOneLastName),
        message: 'You can not select multiple "Last name"',
      },
      {
        validator: value => validateStructureWrapper(value, validateFullNameAndFirstOrLast),
        message: 'You can not select "First name" or "Last name" with "Full name"',
      },
      {
        validator: value => validateStructureWrapper(value, validateAttributeName),
