...
export const receiveChange = ({ // exported for testing
  ...
  // if (
  //   before ||
  //   (!after && !before && cursorHistory.length > 0) // put before if before or after isn't set
  // ) {
  ...
  //   });
  //   cursor = {
  //     before: foundCursor || before /* || (direction === BACKWARDS && loFirst(cursorHistory)) */
  //   };
  // }
  ...
  // does the model exist in the cursors
  ...
  if (existingIndex >= 0) {
    cursor = {
      ...(existingIndex > 0) ? { before: cursorHistory[existingIndex - 1] } : {},
      ...(existingIndex < cursorHistory.length - 1) ? { after: cursorHistory[existingIndex + 1] } : {}
    };
