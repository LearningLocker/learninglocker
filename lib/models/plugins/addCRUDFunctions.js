import _, { map, isArray, isUndefined } from 'lodash';
...
async function getConnectionWs({
  ...
  let oldOldCursor = cursor;
  let oldCursor = cursor;

  this.watch(pipeline.pipeline(), {
    ...
    ws.send(JSON.stringify({
      ...
      node: change,
      cursor: newCursor,
      before: (oldCursor === newCursor) ? oldOldCursor : oldCursor
    }));

    if (oldCursor !== oldOldCursor && newCursor !== oldOldCursor && newCursor !== oldCursor) {
      oldOldCursor = oldCursor;
    }
