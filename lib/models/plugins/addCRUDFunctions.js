...
async function getConnection({
  ...
  if (project) {
    if (scopeSelect) {
      ...
      finalProjection = {};
      keysIntersect.reduce((result, key) => {
        finalProjection[key] = projectKeys[key];
      }, {});
    } else {
  ...
  const startCursor = modelToCursor(_.first(modelsToReturn), sort);
  const endCursor = modelToCursor(_.last(modelsToReturn), sort);
  ...
  const out = {
    edges: _.map(modelsToReturn, model => ({
