...
export const paginationToFilter = ({ cursor, inclusive, sort, paginationDirection }) => {
  const parsedCursor = fromCursor(cursor);
  const sortConditions = _.reduce(
    sort,
    ({ oldKeys, conditions }, direction, key) => {
      const operator = sortDirectionToOperator(direction, paginationDirection, inclusive);
...
export const modelToCursor = (model, sort) => {
...
export const getParsedScopedFilter = async ({
  authInfo,
  filter,
  modelName,
  actionName
