...
schema.statics.writeScopeChecks = function runWriteScopeChecks(req, res, next) {
  ...
  if (
    ...
    && !_.includes(getScopesFromAuthInfo(authInfo), scopes.SITE_ADMIN)
    ...
    req.body = _.map(_.get(req, ['body', 'organisationSettings']), (org) => {
      const out = removeFields(['roles', 'scopes'], org);
      return out;
    });
