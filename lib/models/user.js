...
function validatePassword(value, respond) {
  ...
  if (this.isModified('password')) {
    ...
    if (value.length > 0 && this.ownerOrganisationSettings.PASSWORD_HISTORY_CHECK) {
      let sortedHistory = _.sortBy(this.passwordHistory, 'date');

      // if the length is greater than the allowed total on the owener org settings, trim it down
      if (sortedHistory.length > this.ownerOrganisationSettings.PASSWORD_HISTORY_TOTAL) {
        sortedHistory = _.slice(sortedHistory, sortedHistory.length - this.ownerOrganisationSettings.PASSWORD_HISTORY_TOTAL);
      }
...
schema.writeScopes = [scopes.ALL, MANAGE_ALL_USERS];
...
const whitelistFields = (fields, body) => {
  if (_.isArray(body)) {
    return _.map(body, model => _.pick(model, fields));
  }
...
schema.statics.writeScopeChecks = function runWriteScopeChecks(req, res, next) {
  ...
  if(_.includes(getScopesFromAuthInfo(authInfo), scopes.SITE_ADMIN)){
    return runScopeChecks(model, 'writeScopes', req, res, next);
  }
  
  if ( // if we are our user we can only change our name, and samlEnabled
    authInfo.user &&
    req.params.id && req.params.id.toString() && getUserIdFromAuthInfo(authInfo)
    && req.params.id.toString() === getUserIdFromAuthInfo(authInfo).toString()
  ) {
    req.body = whitelistFields(['name', 'password', 'organisationSettings'], req.body);

    _.set(
      req,
      ['body', 'organisationSettings'],
      _.map(_.get(req, ['body', 'organisationSettings']), (org) => {
        const out = whitelistFields(['samlEnabled'], org);
        return out;
      })
    );
  } else if (
    authInfo.user &&
    _.includes(getScopesFromAuthInfo(authInfo), MANAGE_ALL_USERS)
  ){
    // can manage users
    req.body = whitelistFields(['name', 'organisationSettings'], req.body);

    _.set(
      req,
      ['body', 'organisationSettings'],
      _.map(_.get(req, ['body', 'organisationSettings']), (org) => {
        const out = whitelistFields(['filter', 'roles', 'scopes'], org);
        return out;
      })
    );
  }
