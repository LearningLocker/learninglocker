...
schema.pre('save', async function preSave(next) {
  ...
  // TODO: I think the organisationSettings are never updated.
  ...
  // TODO: I think we can move "creating of roles" into a method for better readability
  ...
  // TODO: I think we can move "updating owner organisations fields" into a method for better readability
  const ownerOrganisationSettings = owner.organisationSettings || [];

  ownerOrganisationSettings.push({
    organisation: this._id,
    roles: [adminRole._id],
  });

  owner.organisations = union(owner.organisations, [this._id]);
  owner.organisationSettings = ownerOrganisationSettings;
...
schema.post('remove', async (org, next) => {
  const Role = getConnection().model('Role');
  const User = getConnection().model('User');

  await Role.deleteMany({ organisation: [org._id] });
  await User.updateMany(
    {},
    {
      $pull: {
        organisations: org._id,
        organisationSettings: { organisation: org._id },
      }
    }
  );
