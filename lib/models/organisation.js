...
const schema = new mongoose.Schema({
  ...
  expiration: {
    type: Date,
    set(newValue) {
      this._oldExpriation = this.expiration;
      return newValue;
    }
  ...
  settings: {
    ...
    PASSWORD_MIN_LENGTH: {
      ...
      validate: {
        isAsync: true,
        validator: minVal.bind(null, 4, 'Passwords must be more than 4 characters')
      }
    },
    ...
    EXPIRE_EXPORTS: {
      allowExportExpirations: { type: Boolean, default: false },
      ttl: { type: Number, default: 15780000 },
      dontAllowExports: { type: Boolean, default: false }
    }
  }
});
...
schema.pre('save', async function preSave(next) {
  ...
  if (!isUndefined(this._oldExpriation) && this.expiration !== this._oldExpriation) {
