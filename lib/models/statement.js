...
export const buildRedisKeyPrefix = (organisationId, pipeline, skip, limit) =>
  cachePrefix(`${organisationId}-AGGREGATION-${sha1(JSON.stringify(pipeline))}-${skip}-${limit}`);
 ...
 * @returns {void}
 */
export const asyncRunAggregation = async (
  redisClient,
  organisationId,
  pipeline,
  skip,
  ...
  const startedAt = moment().format('YYYY-MM-DD HH:mm:ss.SSSZ');

  await Promise.all([
    redisClient.set(`${prefix}-STARTED-AT`, startedAt),
    redisClient.set(`${prefix}-COMPLETED-AT`, undefined),
  ]);
  ...
  await Promise.all([
    redisClient.set(`${prefix}-COMPLETED-AT`, completedAt),
    redisClient.set(`${prefix}-RESULTS`, results),
  ]);
};
 ...
 * @param {number} skip
 * @param {number} limit
 * @returns {object} aggregation status
 */
export const getAggregationStatus = async (redisClient, organisationId, pipeline, skip, limit) => {
  const prefix = buildRedisKeyPrefix(organisationId, pipeline, skip, limit);
 ...
 *                      }
 */
export const asyncAggregate = async (
  authInfo,
  basePipeline,
  skip,
  limit,
  ...
  const aggregationStatus = await getAggregationStatus(redisClient, organisationId, pipeline, skip, limit);

  if (!aggregationStatus.status.startedAt) {
    // not await
    asyncRunAggregation(redisClient, organisationId, pipeline, skip, limit);
  }

  return aggregationStatus;
};

schema.statics.asyncAggregate = asyncAggregate;
