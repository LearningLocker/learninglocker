...
const ASYNC_AGGREGATION_WAITING_MS = defaultTo(Number(process.env.ASYNC_AGGREGATION_WAITING_MS), 1000);
...
const ASYNC_AGGREGATION_REFRESH_AT_SECONDS = defaultTo(Number(process.env.ASYNC_AGGREGATION_REFRESH_AT_SECONDS), 1200); // 20 minutes
 ...
 */
export const getAggregationStatus = async (redisClient, organisationId, pipeline, skip, limit, sinceAt) => {
  const prefix = buildRedisKeyPrefix(organisationId, pipeline, skip, limit);

  const [startedAt, completedAt, results] = await Promise.all([
    redisClient.get(`${prefix}-STARTED-AT`),
    redisClient.get(`${prefix}-COMPLETED-AT`),
    redisClient.get(`${prefix}-RESULTS`),
  ]);

  if (sinceAt && completedAt && moment(completedAt).isBefore(moment(sinceAt))) {
    return {
      result: null,
      startedAt,
      completedAt: null,
 ...
 */
export const decoratePipeline = async (pipeline, authInfo) => {
  const organisation = getOrgFromAuthInfo(authInfo);

  const parsedPipeline = await parseQuery(pipeline, { organisation });
  ...
  if (authInfo.client && authInfo.client.lrs_id) {
    parsedPipeline.unshift({
      $match: { lrs_id: objectId(authInfo.client.lrs_id) }
    });
  }
 ...
 * aggregate Async
 *
 * `cached` means that Redis has cache whose completedAt is after `sinceAt`
 *
 * Case1: no cache + is not running (startedAt is null)
 *   start running aggregation query
 *   wait X milliseconds
 *   if query is finished, return query result
 *     return type is  { result: array, startedAt: string, completedAt: string }
 *   else, return empty result
 *     return type is { result: null, startedAt: string, completedAt: null }
 *
 * Case2: no cache + is running (startedAt is not null)
 *   return type is { result: null, startedAt: string, completedAt: null }
 *
 * Case3: `cached` + is not running (completedAt is same or after startedAt)
 *   start running aggregation query
 *   wait X milliseconds
 *   if query is finished, return new result
 *     return type is { result: array, startedAt: string, completedAt: string }
 *   else, return cached result
 *     return type is { result: array, startedAt: string, completedAt: string }
 *
 * Case4: `cached` + is running (completedAt is before startedAt)
 *   return { result: array, startedAt: string, completedAt: string }
 *
 * @param {*} authInfo
...
export const aggregateAsync = async (
  ...
  const pipeline = await decoratePipeline(basePipeline, authInfo);

  const prefix = buildRedisKeyPrefix(organisationId, pipeline, skip, limit);

  const { result, startedAt, completedAt } = await getAggregationStatus(redisClient, organisationId, pipeline, skip, limit, sinceAt);
  ...
  if (result === null) {
    ...
    // if is not running, start running query
    ...
    const result1 = await getAggregationStatus(redisClient, organisationId, pipeline, skip, limit, sinceAt);
    return result1;
  }

  // if cached
  const bestByDatetime = moment(completedAt).add(ASYNC_AGGREGATION_REFRESH_AT_SECONDS, 'seconds');
  const isFresh = moment().isSameOrBefore(bestByDatetime);

  if (isFresh) {
    return { result, startedAt, completedAt };
  }

  // if cache is old, run aggregation query again
  await redisClient.set(`${prefix}-STARTED-AT`, moment().format('YYYY-MM-DD HH:mm:ss.SSSZ'));
  ...
  const result2 = await getAggregationStatus(redisClient, organisationId, pipeline, skip, limit, sinceAt);
  return result2;
};
