...
const mapDot = (statement) => {
  const statementKeys = mapKeys(statement, (value, key) =>
    key.replace('&46;', '.')
  );

  return mapValues(statementKeys, (value) => {
    if (isObject(value)) {
      return mapDot(value);
    }
    return value;
...
schema.statics.postFetchMap = function (statement) {
