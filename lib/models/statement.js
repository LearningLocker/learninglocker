...
const hasFreshCache = completedAt => {
  ...
  return moment().isSameOrBefore(bestByDatetime);
};
...
export const aggregateAsync = async (
  ...
  // If "no cached and is running" or "has fresh cache"
  const hasNoCacheAndIsRunning = result === null && startedAt;
  if (hasNoCacheAndIsRunning || hasFreshCache(completedAt)) {
