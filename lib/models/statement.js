...
const isRunningCheck = (
  ...
  if (startedAt) {
    ...
    // completed before current start time
    if (moment(completedAt).isBefore(moment(startedAt))) {
      return true;
    }
  }
...
export const getAggregationStatus = async (redisClient, prefix, sinceAt) => {
  ...
  const isRunning = isRunningCheck(startedAt, completedAt);
