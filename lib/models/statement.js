...
/**
 * If not cached OR cache is old
 * start aggregation and wait for ASYNC_AGGREGATION_WAITING_MS milliseconds
 *
 * @param {*} authInfo
 ...
 * }
 *
 * Case 2: cached but it is not fresh
 * {
 *   result: null,
 *   startedAt: string,
 *   completedAt: string,
...
export const aggregateAsync = async (
  ...
  // if is running
  if (moment(completedAt).isSameOrBefore(startedAt)) {
    return {
      result: null,
      startedAt,
      completedAt,
