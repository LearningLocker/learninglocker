...
export const aggregateAsync = async (
  ...
  const { result, startedAt, completedAt } = await getAggregationStatus(redisClient, organisationId, pipeline, skip, limit);

  if (!startedAt) {
    const prefix = buildRedisKeyPrefix(organisationId, pipeline, skip, limit);
    const now = moment().format('YYYY-MM-DD HH:mm:ss.SSSZ');

    await Promise.all([
      redisClient.set(`${prefix}-STARTED-AT`, now),
      redisClient.del(`${prefix}-COMPLETED-AT`),
    ]);
