...
export const aggregateAsync = async (
  ...
  const { result, startedAt, completedAt } = await getAggregationStatus(redisClient, prefix, sinceAt);
  ...
  if (result === null) {
    ...
    const result1 = await getAggregationStatus(redisClient, prefix, sinceAt);
    return result1;
  }
  ...
  const result2 = await getAggregationStatus(redisClient, prefix, sinceAt);
  return result2;
};
