 ...
 * @returns {Promise} aggregation result and status
 */
const _aggregateAsync = async (redisClient, organisationId, pipeline, skip, limit) => {
  const resultAndStatus = await getAggregationStatus(redisClient, organisationId, pipeline, skip, limit);

  if (!resultAndStatus.status.startedAt) {
    const prefix = buildRedisKeyPrefix(organisationId, pipeline, skip, limit);
...
export const aggregateAsync = async (
  ...
  const resultAndStatus = await _aggregateAsync(redisClient, organisationId, pipeline, skip, limit);
  return resultAndStatus;
};
