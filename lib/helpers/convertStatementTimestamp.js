...
const createFilterParser = (key, parser) => {
  return (filter) => {
    if (has(filter, key)) {
      try {
    ...
    if (isPlainObject(filter)) {
      return mapValues(filter, createFilterParser(key, parser));
    }
...
const createDateParser = (oldKey, newKey, mutator) => {
  return createFilterParser(oldKey, dateParser(mutator, oldKey, newKey));
}
