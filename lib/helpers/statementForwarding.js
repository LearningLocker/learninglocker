...
export const getEditableHeaders = (model /* immutable */) => {
  const headers = fromJS(JSON.parse(
    model.getIn(['configuration', 'headers'], '{}')
  ));

  if (headers.get('X-Experience-API-Version')) {
    return headers;
  }
  return headers.set('X-Experience-API-Version', '${statement.version}'); // eslint-disable-line no-template-curly-in-string
};
