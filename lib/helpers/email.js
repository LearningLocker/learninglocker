...
/*
export const sendUpgradeCustomerForm = (inputs) => {
  const subject = 'Upgrade to enterprise enquiry';
  const recipient = 'Lydia Ross <lydia.ross@learningpool.com>';

  const compiledWrapper = _.template(inputs);
  const htmlEmail = compiledWrapper({ title: subject, body: inputs, siteUrl: process.env.SITE_URL });
  const text = htmlToText.fromString(htmlEmail);
