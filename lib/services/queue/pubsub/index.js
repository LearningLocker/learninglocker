...
const getPubsubClient = async () => {
  ...
  pubsubClient = await pubSub({
    keyFilename: process.env.GOOGLE_CLOUD_PUBSUB_KEY_FILENAME,
    projectId: process.env.GOOGLE_CLOUD_PUBSUB_PROJECT_ID,
  });
  return pubsubClient;
};

const SUBSCRIPTION_NAME = defaultTo(process.env.GOOGLE_CLOUD_PUBSUB_SUBSCRIPTION_NAME, 'll');
