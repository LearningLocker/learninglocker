...
const clientDeleteFilter = (scopes, filter) => {
  // Append the LRS ID to the filter if present
  const clientLrs_id = get(authInfo, 'client.lrs_id', false);
  if (clientLrs_id) {
    return {
      ...filter,
      lrs_id: objectId(clientLrs_id)
    };
  }
...
const clientFilter = (actionName, clientId, scopes, filter) => {
  switch (actionName) {
    case 'delete':
      return clientDeleteFilter(scopes, filter)
    default:
...
export default async ({ actionName, authInfo, allowDashboardAccess = false }) => {
  ...
  switch (tokenType) {
    ...
    case 'dashboard':
      ...
      return filter;
    case 'client':
      const clientId = authInfo.client._id;
      return clientFilter(actionName, clientId, scopes, filter);
    default:
      return filter;
  }
};
