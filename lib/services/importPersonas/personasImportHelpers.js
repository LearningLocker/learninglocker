...
export const getIfis = ({
  ...
  const ifis = ifiKeyColumns.map(({
    ...
    if (columnType === COLUMN_ACCOUNT_VALUE && get(row, 'useConstant', false) === false) {
      if ( // ignore un set rows in the csv
        !row[columnName] ||
        !row[relatedColumn]
      ) {
    ...
    if (columnType === COLUMN_ACCOUNT_VALUE && get(row, 'useConstant', false) === true /* && [implied] && useConstant === true */) {
      if (!row[columnName]) {
        return null;
      }
      return {
