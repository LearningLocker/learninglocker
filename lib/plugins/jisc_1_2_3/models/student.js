...
import domiciles from 'lib/plugins/jisc_1_2_3/utils/domiciles';
import ethnicityCodes from 'lib/plugins/jisc_1_2_3/utils/ethnicityCodes';
...
const schema = new mongoose.Schema({
  ...
  LAST_NAME: { type: String, required: true, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.STUDENT_APP, scopes.TRIBAL_INSIGHT, scopes.SSP, scopes.OPENDASH] },
  FIRST_NAME: { type: String, required: true, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.STUDENT_APP, scopes.TRIBAL_INSIGHT, scopes.SSP, scopes.OPENDASH] },
  ...
  TUTOR_STAFF_ID: { type: String, required: true, maxLength: 255, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.TRIBAL_INSIGHT, scopes.SSP, scopes.OPENDASH] },
...
const updateForeignRelations = (model, next) => {
  const StudentAssessmentInstance = getConnection('JISC_1_2_3').model('StudentAssessmentInstance');
  const StudentCourseInstance = getConnection('JISC_1_2_3').model('StudentCourseInstance');
  ...
  const Institution = getConnection('JISC_1_2_3').model('Institution');
...
export default getConnection('JISC_1_2_3').model('Student', schema, 'jiscStudents');
