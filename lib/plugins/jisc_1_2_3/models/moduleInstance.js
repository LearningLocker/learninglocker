...
const schema = new mongoose.Schema({
  ...
  MOD_ACADEMIC_YEAR: { type: Number, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.TRIBAL_INSIGHT, scopes.LAP, scopes.SSP, scopes.OPENDASH] },
  MOD_OPTIONAL: { type: Number, required: true, min: 1, max: 2, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.TRIBAL_INSIGHT, scopes.LAP, scopes.SSP, scopes.OPENDASH] },
  MOD_LOCATION: { type: String, required: false, maxLength: 255, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.TRIBAL_INSIGHT, scopes.LAP, scopes.SSP, scopes.OPENDASH] },
...
const updateForeignRelations = (model, next) => {
  const Module = getConnection('JISC_1_2_3').model('Module');
  const AssessmentInstance = getConnection('JISC_1_2_3').model('AssessmentInstance');
  const ModuleVleMap = getConnection('JISC_1_2_3').model('ModuleVleMap');
...
export default getConnection('JISC_1_2_3').model('ModuleInstance', schema, 'jiscModuleInstances');
