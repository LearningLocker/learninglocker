...
const schema = new mongoose.Schema({
  ...
  FIRST_NAME: { type: String, maxLength: 255, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.TRIBAL_INSIGHT, scopes.SSP, scopes.OPENDASH] },
  LAST_NAME: { type: String, maxLength: 255, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.TRIBAL_INSIGHT, scopes.SSP, scopes.OPENDASH] },
...
const updateForeignRelations = (model, removeRelations, next) => {
  ...
  const StaffModuleInstance = getConnection('JISC_1_2_7').model('StaffModuleInstance');
  ...
  async.parallel([
    ...
    done => updateForeignSingular(StaffCourseInstance, model, removeRelations, query, 'staff', done),

    // 'StaffModuleInstance.staff'
    done => updateForeignSingular(StaffModuleInstance, model, removeRelations, query, 'staff', done),

    // 'Student.tutor'
    done => updateForeignSingular(Student, model, removeRelations, {
...
export default getConnection('JISC_1_2_7').model('Staff', schema, 'jiscStaff');
