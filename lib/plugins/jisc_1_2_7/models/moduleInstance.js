...
const schema = new mongoose.Schema({
  ...
  MOD_ONLINE: { type: Number, min: 1, max: 2, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.TRIBAL_INSIGHT, scopes.LAP, scopes.SSP, scopes.OPENDASH] },
...
const updateForeignRelations = (model, removeRelations, next) => {
  const Module = getConnection('JISC_1_2_7').model('Module');
  const AssessmentInstance = getConnection('JISC_1_2_7').model('AssessmentInstance');
  const ModuleVleMap = getConnection('JISC_1_2_7').model('ModuleVleMap');
...
export default getConnection('JISC_1_2_7').model('ModuleInstance', schema, 'jiscModuleInstances');
