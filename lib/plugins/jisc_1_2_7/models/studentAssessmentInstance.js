...
const schema = new mongoose.Schema({
  ...
  X_MOD_ACADEMIC_YEAR: { type: Number, max: 9999, readAccess: [scopes.ALL, scopes.UDD_READ, scopes.STUDENT_APP, scopes.LAP, scopes.SSP] },
...
schema.index({ organisation: 1, STUDENT_COURSE_MEMBERSHIP_ID: 1, ASSESS_INSTANCE_ID: 1, ASSESS_SEQ_ID: 1, STUDENT_COURSE_MEMBERSHIP_SEQ: 1 }, { unique: true });
...
const updateForeignRelations = (model, removeRelations, next) => {
  ...
  const AssessmentInstance = getConnection('JISC_1_2_7').model('AssessmentInstance');
  ...
  async.parallel([
    ...
    done => updateForeignMultiple(AssessmentInstance, model, removeRelations, {
...
export default getConnection('JISC_1_2_7').model('StudentAssessmentInstance', schema, 'jiscStudentAssessmentInstances');
