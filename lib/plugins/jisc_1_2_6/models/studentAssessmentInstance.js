...
schema.index({ organisation: 1, STUDENT_ID: 1 });
schema.index({ organisation: 1, MOD_INSTANCE_ID: 1 });

const updateForeignRelations = (model, removeRelations, next) => {
  const Student = getConnection('JISC_1_2_6').model('Student');
  const StudentCourseMembership = getConnection('JISC_1_2_6').model('StudentCourseMembership');
...
export default getConnection('JISC_1_2_6').model('StudentAssessmentInstance', schema, 'jiscStudentAssessmentInstances');
