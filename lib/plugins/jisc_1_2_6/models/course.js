...
const updateForeignRelations = (model, removeRelations, next) => {
  ...
  const Institution = getConnection('JISC_1_2_6').model('Institution');
  ...
  async.parallel([
    ...
    done => updateForeignSingular(CourseInstance, model, removeRelations, query, 'course', done),

    // 'Institution.course'
    done => updateForeignSingular(StudentCourseMembership, model, removeRelations, query, 'course', done),
...
export default getConnection('JISC_1_2_6').model('Course', schema, 'jiscCourses');
