define([
  ...
  return backbone.Model.extend({
    ...
    _getGravatar: function (email) {
      if (email) {
        return 'http://www.gravatar.com/avatar/' + MD5(email) + '.jpg?s=' + this._gravatarSize;
      } else {
    ...
    _diffForHumans: function (date) {
      ...
      } else if ((interval = Math.floor(seconds / 604800)) > 1) {
    ...
    parse: function (response) {
      response.gravatar = this._getGravatar(
        response.statement.actor.mbox.replace('mailto:', '')
      );
      response.timestamp = new Date(response.statement.timestamp);
      response.when = this._diffForHumans(response.timestamp);
      return response;
    }
  });
});