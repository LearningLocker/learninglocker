...
class Component extends React.Component<Props, State> {
  ...
  handleDataChange() {
    var data = this.props.service.getResult().map(function (agent) {
      var id = '';

      if (agent._id.mbox) {
        id = agent._id.mbox
      } else if (agent._id.account_name) {
        id = 'account:' + agent._id.account_homePage + '/' + agent._id.account_name
      } else if (agent._id.openid) {
        id = 'openid:' + agent._id.openid
      } else if (agent._id.mbox_sha1sum) {
        id = 'sha:' + agent._id.mbox_sha1sum
      } 

      return (agent.name || 'Unknown') + ' (' + id + ')';
    });
