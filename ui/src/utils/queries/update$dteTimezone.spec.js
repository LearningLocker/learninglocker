...
test('update timezone reflects DST', () => {
  const query = fromJS({
    $and: [
      {
        $or: [
  ...
  expect(actual.getIn(['$and', 2, 'stored', '$timestamp', '$dte'])).toEqual('2018-03-25T00:00+00:00');
  expect(actual.getIn(['$and', 3, 'timestamp', '$timestamp', '$dte'])).toEqual('2018-03-26T00:00+01:00');
  expect(actual.equals(expected)).toEqual(true);
});
